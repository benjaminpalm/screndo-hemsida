...
export default function Home() {
  ...
  const cost = useFadeIn(0.1);
  const problem = useFadeIn();
  const ctaSection = useFadeIn();
  const faq = useFadeIn();
  const [costCount, setCostCount] = useState(0);
  const costCountStarted = useRef(false);
  useEffect(() => {
    if (cost.visible && !costCountStarted.current) {
      costCountStarted.current = true;
      let n = 0;
      const interval = setInterval(() => {
        n += 1;
        setCostCount(n);
        if (n >= 52) clearInterval(interval);
      }, 30);
    }
  }, [cost.visible]);
    ...
    <>
      ...
      <section style={{ background: "#ffffff", padding: "100px 0" }}>
        ...
        >
          ...
          {/* Headline */}
          <h2 style={{
            ...fadeStyle(cost.visible, 0.1, 0.6),
            fontFamily: "var(--font-playfair)",
            fontSize: "48px",
            fontWeight: 700,
            lineHeight: 1.15,
          ...
          {/* Large count-up stat */}
          <div style={{
            ...fadeStyle(cost.visible, 0.2, 0.5),
            fontFamily: "var(--font-playfair)",
            fontSize: "96px",
            fontWeight: 700,
            lineHeight: 1,
          ...
          {/* Stat description */}
          <p style={{
            ...fadeStyle(cost.visible, 0.3, 0.5),
            color: "#2C1810",
            fontSize: "18px",
            lineHeight: 1.7,
            margin: "0 0 16px",
          ...
          {/* Stat note */}
          <p style={{
            ...fadeStyle(cost.visible, 0.4, 0.5),
            color: "#8B7355",
            fontSize: "13px",
            fontStyle: "italic",
            lineHeight: 1.7,
          ...
          {/* Divider */}
          <div style={{
            ...fadeStyle(cost.visible, 0.45, 0.4),
            ...
            {t.costCards.map((card, i) => (
              <div
                ...
                style={{
                  ...
                  opacity: cost.visible ? 1 : 0,
                  transform: cost.visible ? "translateY(0)" : "translateY(24px)",
                  transition: `opacity 0.55s ease-out ${0.5 + i * 0.15}s, transform 0.55s ease-out ${0.5 + i * 0.15}s`,
                }}
              >
                <div style={{
                  fontFamily: "var(--font-playfair)",
